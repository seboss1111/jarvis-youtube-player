...
jv_pg_yp_play_song()
{
  ...
  if [[ $2 =~ "True" ]]; then
    youtube-dl --max-downloads 1 --format mp4 --output "/tmp/$1_audio.mp4" "gvsearch1:$1" && mplayer -novideo -ao alsa:device=hw=$var_jv_pg_yp_audio_card.$var_jv_pg_yp_audio_peripheral "/tmp/$1_audio.mp4" && rm "/tmp/$1_audio.mp4" &
  else
...
jv_pg_yp_play_video()
{
  ...
  if [[ $2 =~ "True" ]]; then
    youtube-dl --max-downloads 1 --format mp4 --output "/tmp/$1_video.mp4" "gvsearch1:$1" && mplayer $full_screen -vf scale -zoom -ao alsa:device=hw=$var_jv_pg_yp_audio_card.$var_jv_pg_yp_audio_peripheral "/tmp/$1_video.mp4" && rm "/tmp/$1_video.mp4" &
  else
